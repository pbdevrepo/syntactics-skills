...
When the same concept appears under different names (e.g., "users" in one section and "members" in
another), flag the inconsistency internally. Surface it in Phase 3 - do not silently pick one.
...
**Sharpen fuzzy terms before closing questions.** If the client input uses a vague or overloaded term,
include a sharpening question that proposes a precise interpretation. Do not leave it for grilling to catch.

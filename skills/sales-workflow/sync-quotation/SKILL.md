...
2. Read the full proposal - every module in the Scope of Work must appear in the quotation.
...
- Never combine Design + Frontend + Backend into one sub-item - separate them.
- Always include setup and scaffolding tasks (project setup, DB migration, authentication base) as their own sub-items.
- For CRUD modules: separate Create form, List view, Detail view, Edit, and Delete into sub-items if they differ in complexity.
...
- A sub-item only gets hours for the roles that touch it - do not assign Backend hours to a purely UI sub-item.
- When complexity is genuinely unknown, use the `Unknown` range from `references/hour-ranges.md` and flag it with `[Review needed]`.
- Do not compress ranges to make totals look smaller. Ranges exist so Sales can see the spread and adjust with confidence.
...
- [ ] Every module from the proposal appears in the quotation - none skipped
- [ ] Every sub-item has a role tag
- [ ] Every sub-item has a placeholder hour range
- [ ] No sub-item exceeds 40 hrs on the high end without a split note explaining why
- [ ] Module totals and grand total are calculated correctly
- [ ] All `Unknown` ranges are flagged with `[Review needed]` for Sales
- [ ] No single-number estimates appear anywhere - ranges only

...
### Added
- `engineering-workflow`: `sync-dev-tdd-stage` `agents/engineering/tdd-qa-tester.md` - RED phase specialist agent: writes exactly one failing test per delegation, classifies it (Happy Path / Sad Path / Edge Case / Mocked Boundary), confirms RED before returning; used by sync-dev-tdd-stage orchestrator for every RED phase
- `engineering-workflow`: `sync-dev-tdd-stage` `agents/engineering/tdd-backend-developer.md` - GREEN phase specialist for backend service layer and business logic; writes minimum implementation to pass the failing test, runs tsc and ESLint hygiene, confirms all tests GREEN before returning
- `engineering-workflow`: `sync-dev-tdd-stage` `agents/engineering/tdd-frontend-developer.md` - GREEN phase specialist for UI components and state; writes minimum implementation, runs tsc, ESLint, and axe.run hygiene, confirms all tests GREEN before returning
- `engineering-workflow`: `sync-dev-tdd-stage` `agents/engineering/tdd-api-designer.md` - API design specialist: designs REST/GraphQL specifications, OpenAPI 3.1 schemas, auth modeling, and versioning strategies; for BE/FS sessions with dedicated API route tasks
- `engineering-workflow`: `sync-dev-tdd-stage` `agents/engineering/tdd-fullstack-developer.md` - GREEN phase specialist for features spanning backend and frontend; runs hygiene checks across both layers
...
### Changed
- `engineering-workflow`: `sync-dev-tdd-stage` v1.5.0 -> v1.6.0 - agent orchestration: Steps 2-3 now delegate RED to `tdd-qa-tester` and GREEN to `tdd-{type}-developer`; added Agent Selection table in Step 0 mapping session type to developer agent; added `## Agent Delegation Protocol` section with orchestrator/agent ownership split and delegation message format; added Execution Rule 5 (Agent Delegation Integrity)
- `engineering-workflow`: `sync-dev-tdd` v1.4.0 -> v1.5.0 - replaced abandoned `jest-axe` with `axe-core` (actively maintained, MPL-2.0, no high-severity vulns): Tier 2 source table and file-based config detection updated from `jest-axe`/`testing:a11y-component` to `axe-core`/`testing:a11y`; enforcement rule updated to cover both layers - component-level `axe.run(container)` in JSDOM (with `color-contrast` disabled) and E2E inject via `page.addScriptTag`; per-GREEN hygiene and Checklist Per Cycle updated; E2E Coverage section now names the inject pattern explicitly; Philosophy section trimmed to single "Vertical slices only" constraint
- `engineering-workflow`: `sync-dev-tdd` `references/a11y.md` - replaced jest-axe/axe-playwright two-package model with single `axe-core` package; component section updated to `axe.run(container)` with JSDOM `color-contrast` disable note; page-level section updated to `page.addScriptTag({ path: require.resolve('axe-core') })` inject-and-run pattern
- `engineering-workflow`: `sync-dev-tdd` `references/swagger-output-format.md` - added `## Contents` TOC; split monolithic 261-line YAML block into three headed sections (Info and servers, Paths - CRUD endpoints, Components) to prevent partial reads missing component schemas; fixed em dashes to hyphens in title and YAML description string
